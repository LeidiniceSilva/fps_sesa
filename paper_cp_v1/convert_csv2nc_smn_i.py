...
# Reading smn station
df = pd.read_csv(os.path.join('/home/mda_silv/users/FPS_SESA/database/obs/smn_i/', 'PP_SMN_MIN_2018-01-01_2021-12-31.csv'))
df['Date'] = pd.to_datetime(df['Date'])
df = df.set_index('Date').resample('H').sum()
df.iloc[:,:].to_csv('/home/mda_silv/users/FPS_SESA/database/obs/smn_i/PP_SMN_H_2018-01-01_2021-12-31.csv')
...
for idx in range(0, 72):
	...
	# Reading smn station
	data = np.loadtxt(os.path.join('/home/mda_silv/users/FPS_SESA/database/obs/smn_i/', 'PP_SMN_H_2018-01-01_2021-12-31.csv'), dtype='str', delimiter=',', unpack=True)
	data = data[:,1:]
	...
	nc_output = '/home/mda_silv/users/FPS_SESA/database/obs/smn_i/smn_nc/pre_{0}_H_2018-01-01_2021-12-31.nc'.format(smn_i[idx+1][0])
